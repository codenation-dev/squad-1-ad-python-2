...
class Commission_plan(models.Model):
    lower_percentage = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name="lo")
    upper_percentage = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name="up")
    min_value = models.DecimalField(
        max_digits=19, decimal_places=2, verbose_name="min")
...
class Sellers(models.Model):
    ...
    plan = models.ForeignKey(
        Commission_plan, on_delete=models.CASCADE, verbose_name="plan")
...
class Sales(models.Model):
    ...
    sellers_id = models.ForeignKey(
        Sellers, on_delete=models.CASCADE, primary_key=False, verbose_name="sid")
    ...
    def calc_commission(self, seller, amount):
        if not Sellers.objects.filter(id=seller):
            ...
            if dec_amount <= MyDecimal(sel_calc.plan.min_value):
                commission = dec_amount * \
                    MyDecimal(sel_calc.plan.lower_percentage / 100)
            else:
                commission = dec_amount * \
                    MyDecimal(sel_calc.plan.upper_percentage / 100)
    ...
    def notify_seller(self, seller, month):
        if not Sales.objects.filter(sellers_id=seller, month=month):
            ...
            send_mail(
                ...
                [Sales.objects.get(sellers_id=seller,
                                   month=month).sellers_id.email],
                fail_silently=False
            )

    def check_commission(self, seller, amount):
        month = datetime.datetime.now().month
        db_fetch = Sales.objects.select_related(
            'sellers_id').filter(sellers_id=seller)
        if not db_fetch:
            return "Vendedor não cadastrado. Favor verificar os dados de entrada."
        else:
